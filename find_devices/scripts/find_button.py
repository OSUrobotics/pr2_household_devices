...
def find_button(request):
    """ Do all the things for finding a button with human help. """

    ########  Find Buttons with Human Help #########
    ...
    while grab_pt.need_image:
        rospy.sleep(0.01)
    point_from_pixel = PointFromPixel('/head_mount_kinect/depth_registered/camera_info', '/head_mount_kinect/depth_registered/image_rect')
    ...
    pt_button = []  # list of 3D button locations
    names_button = []
    while grab_pt.need_point:
        ...
        if not grab_pt.need_point:  # if we got a point
            pt_button.append(point_from_pixel.calculate_3d_point(grab_pt.point))  # get 3d point that corresponds to pixel
            names_button.append(raw_input('Name this button: '))
        grab_pt.need_point = not grab_pt.need_point  # if got a point, loop; else continue

    # Assemble response to service request
    response = FindButtonResponse()
    for pt, name in zip(pt_button, names_button):
...
if __name__ == '__main__':
    ...
    rospy.init_node('find_button')
    srv = rospy.Service('find_button', FindButton, find_button)
    srv.spin()
